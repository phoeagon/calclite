...
double  grammar ::  statement() {           /**R*/  /** assignment = */
    ...
    if (data.first!=_assign_type){/**  if next operator is not assignment */
            tkin.putback();     tkin.putback();/** put back the variable and operator */
            return boolean_expression();
        }
    ...
    while(1){
        switch( (int)data.second ){
            ...
            case '/':{           /** division */
                ...
                if (equal(dvsr,0)){throw divide_zero() ;}/**reject 0 divisor*/
                left /= dvsr;
                data = tkin.get_token();
                break;
            }
            case '%':{           /** modulus */ /** requires integer-value*/
            ...
            case shift_opr('P'):{/** permutation - arrangement*//** int value*/
                int val1 = force_int(left);
                int val2 = force_int(unary_plus());
                return permutation(val1,val2);
            }
            case shift_opr('G'):{/** gcd *//** int value*/
                int val1 = force_int(left);
                int val2 = force_int(unary_plus());
                return gcd(val1,val2);
            }
            case shift_opr('L'):{/** lcm *//** int value*/
                int val1 = force_int(left);
                int val2 = force_int(unary_plus());
...
double  grammar ::   unary_plus(){          /**R*/  /** <unary> + - @(bitwise not) ~(logic not)*/
    ...
    if ( t.first == _opr_type ){
        switch( (int)(t.second) ){
            ...
            case '-':   return -unary_plus();
            case '@':   return ~force_int(unary_plus());    /**int value*/
            case '~':   return !force_int(unary_plus());    /**int value*/
            default :   ;
        }
    }
